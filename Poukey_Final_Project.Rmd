...
At the base level, centrality measures how important, or central, a node is in a given network. This measurement of importance can be found through several different methods, such as degree centrality, eigenvector centrality, and betweenness centrality. In this analysis, we will explore each centrality and provide insights 
...
DESCRIBE OVERVIEW HERE

```{r, fig.height=6, fig.cap = 'Directed Network of ECESS Board Relationships Grouped By Strength'}
G_friendship %>%
  activate(edges) %>%
  mutate(Relationship = as_factor(Friendship)) %>%
...
DESCRIBE FACTTED GRAPH HERE

```{r, fig.cap = 'Directed Friendship Network With Degree Centralities of ECESS Board Members'}
G_friendship %>%
  activate(edges) %>%
  filter(Friendship > 0) %>%
...
ANALYZE POPULARITY AND IMAGE HERE
...
DESCRIBE EIGENVECTOR GRAPH HERE
...
ANALYZE INFLUENCE AND COMMITTMENT HERE
...
ANALYZE CONNECTIVITY AND INFORMATION DIFFUSION HERE
